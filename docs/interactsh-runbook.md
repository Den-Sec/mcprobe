...
mcpsnare confirms blind command-injection / SSRF via an interactsh / OAST server.

## Out of the box

mcpsnare ships a real interactsh client (`mcpsnare.oob.interactsh_client.InteractshClient`)
implementing the interactsh **RSA-OAEP(SHA-256) + AES-256-CTR** protocol. Just select it:

    mcpsnare scan --http https://target/mcp --oob interactsh
...
self-hosted interactsh server) with `--interactsh-server`:
    ...
    mcpsnare scan --http https://target/mcp --oob interactsh --interactsh-server my.oast.server

A vulnerable target that executes an injected `curl http://<token>.<oast-domain>` makes a
DNS/HTTP callback that interactsh records; mcpsnare's poll-until-hit loop catches it and
...
- The full path has been **manually verified live against the public `oast.fun`**: a real
  DNS callback to a mcpsnare-generated subdomain was registered, polled, decrypted, and
