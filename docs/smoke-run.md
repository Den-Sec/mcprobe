...
mcpsnare is exercised in CI against bundled deliberately-vulnerable fixtures
...
reference server - to confirm mcpsnare handles real-world schemas, enumerates real
tools/resources, and produces sane output without crashing or false-positing on a
benign target.

## What was scanned

`@modelcontextprotocol/server-everything` (run via `npx -y`) exposes, as seen by mcpsnare:

- **13 tools** with **4 string injection points** (mcpsnare's schema-aware mapper walked
...
mcpsnare scan --stdio "npx -y @modelcontextprotocol/server-everything" --oob none
-> 0 finding(s)   (exit 0)

# aggressive mode (adds blocking time-based cmd/SQL probes), JSON output
mcpsnare scan --stdio "npx -y @modelcontextprotocol/server-everything" \
...
The v1 review's core critique was that mcpsnare "works on the toy fixture but is shallow
